...
class Events(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.CommandNotFound):
            ...
            await ctx.send(f"# ❌ Invalid argument(s).\nPlease check your argument(s).\n-# {utils.little_error_variant()}")
            return
            ...
            await ctx.send(f"# ❌ This command is currently disabled.\n-# Well that's pretty rare, seems like that command was too dangerous for me to keep using..")
            return
        elif isinstance(error, NoPrivateMessage):
            await ctx.send(f"# ❌ This command cannot be used in private messages.\n-# Hey um, just asking to not use commands in private messages since that doesn't work probably.")
            return
