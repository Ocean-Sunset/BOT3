...
class Utility(commands.Cog):
    ...
    @app_commands.command(
        name="setlivestream",
        description="Set the YouTube channel ID to monitor for livestreams."
    )
    @utils.admin_or_owner()
    async def set_livestream(self, interaction: discord.Interaction, channel_id: str):
        """Set the YouTube channel ID to monitor for livestreams."""
        utils.set_youtube_channel_id(interaction.guild.id, channel_id)
        await interaction.response.send_message(f"✅ YouTube channel ID set to `{channel_id}` for livestream monitoring.")

    @app_commands.command(
        name="setlivestreamchannel",
        description="Set the Discord channel for livestream announcements."
    )
    @utils.admin_or_owner()
    async def set_livestream_channel(self, interaction: discord.Interaction, channel: typing.Optional[discord.TextChannel] = None):
        """Set the Discord channel for livestream announcements."""
        if channel is None:
            channel = interaction.channel
        utils.set_livestream_channel_id(interaction.guild.id, channel.id)
        await interaction.response.send_message(f"✅ Livestream announcement channel set to {channel.mention}.")

    @app_commands.command(
        name="checklivestream",
        description="Check if the set YouTube channel is currently live and announce if so."
    )
    @utils.admin_or_owner()
    async def check_livestream(self, interaction: discord.Interaction):
        """Check if the set YouTube channel is currently live and announce if so."""
        youtube_channel_id = utils.get_youtube_channel_id(interaction.guild.id)
        if not youtube_channel_id:
            await interaction.response.send_message("# ❌ No YouTube channel ID set. Use `/setlivestream <channel_id>` first.")
            return
        livestream_channel_id = utils.get_livestream_channel_id(interaction.guild.id)
        announce_channel = None
        if livestream_channel_id:
            announce_channel = self.bot.get_channel(livestream_channel_id)
        if not announce_channel:
            announce_channel = interaction.channel  # Fallback to current channel

        api_key = "AIzaSyC3ik7r-4UX6eITy_Fn2orsLadA0mWt7uE"  # Replace with actual API key
        url = (
            f"https://www.googleapis.com/youtube/v3/search"
            f"?key={api_key}&channelId={youtube_channel_id}&part=snippet,id&eventType=live&type=video&maxResults=1"
        )
        try:
            response = requests.get(url)
            data = response.json()
            if "items" in data and data["items"]:
                ...
                embed = discord.Embed(
                    title=f"🔴 LIVE: {title}",
                    url=video_url,
                    description=f"The channel is currently live!\nWatch here: {video_url}",
                    color=discord.Color.red()
                )
                embed.set_thumbnail(url=video["snippet"]["thumbnails"]["high"]["url"])
                await announce_channel.send(embed=embed)
                await interaction.response.send_message("✅ Livestream detected and announced!")
            else:
                await interaction.response.send_message("❌ No active livestream found.")
        except Exception as e:
            await interaction.response.send_message(f"# ❌ Failed to check livestream: {e}")
