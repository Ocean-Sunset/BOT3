...
class Members(commands.Cog, name="Members"):
    ...
    async def role(self, interaction: discord.Interaction, member: discord.Member, role: discord.Role):
        ...
        if role >= interaction.user.top_role and interaction.user != interaction.guild.owner:
            return await interaction.response.send_message(
                embed=EmbedBuilder().title("Cannot Manage").description("You cannot manage this role.").color("red").timestamp(datetime.datetime.utcnow()).build(),
                ephemeral=True
            )
        if role in member.roles:
            await member.remove_roles(role, reason=f"Removed by {interaction.user}")
            await interaction.response.send_message(
                embed=EmbedBuilder().title("Role Removed").description(f"Removed {role.mention} from {member.mention}.").color("orange").field("Moderator", interaction.user.mention).timestamp(datetime.datetime.utcnow()).build(),
                ephemeral=True
            )
        else:
            await member.add_roles(role, reason=f"Added by {interaction.user}")
            await interaction.response.send_message(
                embed=EmbedBuilder().title("Role Added").description(f"Added {role.mention} to {member.mention}.").color("green").field("Moderator", interaction.user.mention).timestamp(datetime.datetime.utcnow()).build(),
                ephemeral=True
            )
    ...
    async def warnings(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if not rows:
            return await interaction.response.send_message(
                embed=EmbedBuilder().title("No Warnings").description(f"{member.mention} has no warnings.").color("blue").timestamp(datetime.datetime.utcnow()).build(),
                ephemeral=True
            )
