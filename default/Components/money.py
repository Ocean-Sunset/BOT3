...
class Money(commands.Cog):
    ...
    async def buylevel(self, ctx, levels: str):
        ...
        level_cost = 50  # Example cost per level
        ...
        user_data["coins"] -= bought * level_cost
        user_data["level"] += bought
        utils.update_user_data(user_id, "coins", user_data["coins"])
        utils.update_user_data(user_id, "level", user_data["level"])
        await ctx.send(f"✅ You bought {bought} level(s)! You are now level {user_data['level']}.")
