...
class SocialAlerts(commands.Cog, name="SocialAlerts"):
    ...
    async def set_youtube(self, interaction: discord.Interaction, youtube_channel_id: str, ping_role: Optional[discord.Role] = None, announce_channel: Optional[discord.TextChannel] = None):
        ...
        settings["announce_channel_id"] = str(announce_channel.id) if announce_channel else str(interaction.channel_id)
    ...
    @app_commands.describe(handle="Twitter/X handle (without @)", ping_role="Role to ping on post")
    async def set_twitter(self, interaction: discord.Interaction, handle: str, ping_role: Optional[discord.Role] = None):
        if not interaction.user.guild_permissions.manage_guild:
            return await interaction.response.send_message(
                embed=EmbedBuilder().title("Permission Denied").description("You need Manage Server permission.").color("red").timestamp(datetime.datetime.utcnow()).build(),
                ephemeral=True
    ...
    async def config(self, interaction: discord.Interaction):
        ...
        announce = interaction.guild.get_channel(int(announce_id)) if announce_id else None
        embed = (
            EmbedBuilder()
            ...
            .field("Announce Channel", announce.mention if announce else "Not set")
            .timestamp(datetime.datetime.utcnow())
            .build()
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
