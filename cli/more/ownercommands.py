...
class Ownercommands(commands.Cog):
    ...
    @app_commands.command(name="update", description="Update the bot's version and new features, then restart.")
    @app_commands.checks.has_permissions(administrator=True)
    @app_commands.describe(version="The new version", new_stuff="New features", insider="Is this for insider?")
    async def update(self, interaction: discord.Interaction, version: str, new_stuff: str, insider: bool = False):
        ...
        if insider:
            ...
            update_text = f"📣 Bot updated to **{version}**!\nWhat's new: {new_stuff}"
            for guild in self.bot.guilds:
                try:
                    try:
                        if not utils.get_guild_setting(guild.id, "general_updates_enabled", True):
                            continue
                    except Exception:
                        pass
                    best_channel = None
                    ...
                    if best_channel and best_score > 0:
                        await best_channel.send(update_text)
                except Exception:
                    continue
