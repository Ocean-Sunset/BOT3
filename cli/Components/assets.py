...
ASSETS_BASE_DIR = "BOT3/default/assets"
...
class AssetManager(commands.Cog):
    ...
    async def upload_asset(self, interaction: discord.Interaction, filename: str, attachment: discord.Attachment):
        ...
        await interaction.response.send_message(f"# ⚠️ Hey!\nPlease make sure this is .PNG file, and that it's 800x288! if one of those conditions are not checked, and that the upload is still going, contact our developper.\n-# name's th3_t1sm if you didn't know, yeah.. good luck i guess???", ephemeral=True)
