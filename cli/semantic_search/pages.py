...
keywords). The semantic search service embeds the *text derived from this
metadata* (never raw HTML) and compares it against the incoming query.

Keep this in sync with the Vercel SEARCH_CATALOG (website/api/index.py). The
"panel" value is the URL slug used by both sides so results can be joined.
"""
