...
CATEGORY_MAPPING = {
    ...
    "ℹ️ Information": [
        "profile", "info", "changelog", "analyse", "serverstats", "setannouncements", "migrate"
    ],
    "🚨 Owner only commands": [
    ...
    "📝 Miscellaneous": [
        ...
        "submitidea", "feedback", "crashreport",
    ],
...
class Help(commands.Cog):
    ...
    @app_commands.command(name="help", description="Show the bot's help menu or info about a command.")
    async def help(self, interaction: discord.Interaction, query: str = None):
        ...
        if query:
            ...
            command = discord.utils.find(lambda c: c.name == query, app_commands.command)
            if not command:
                # Fuzzy Search Fallback for slash commands
                matches = [
                    cmd for cmd in app_commands.command
        ...
        for cmd in app_commands.command:
